...
const EditActivityForm = ({ navigation, route }) => {
   ...
   useEffect(() => {
      navigation.setOptions({
         headerRight: () => (
            ...
            >
               <Text style={GlobalStyle.HeaderButtonText}>Lưu</Text>
            </TouchableOpacity>
         ),
      });
   }, [navigation, activity]);
      ...
      const loadActivity = async () => {
         ...
         try {
            ...
            if (response.status === statusCode.HTTP_200_OK) {
               setActivity(mapValues(activity, response.data));
            }
         } catch (error) {
   ...
   const handleEditActivity = async () => {
      ...
      try {
         let response = await authAPI(accessToken).patch(endPoints['activity-detail'](activityID), form);
   ...
   const mapValues = (initialActivity, apiData) => {
               ...
               <View style={ActivitySettingsFormStyle.InputRow}>
                  <View style={ActivitySettingsFormStyle.InputContainer}>
                     ...
                     <View style={ActivitySettingsFormStyle.ButtonContainer}>
                        <TouchableOpacity style={ActivitySettingsFormStyle.Button} onPress={handleDeleteActivity}>
